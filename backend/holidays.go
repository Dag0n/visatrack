...
// ukBankHolidays is the England & Wales bank holiday calendar published by
// gov.uk (https://www.gov.uk/bank-holidays.json), used so that "first working
// day" calculations match UKVI's definition of processing time. Dates beyond
// the next year or so are gov.uk's forward calculated/estimated holidays —
// re-fetch from the source when this list runs out.
var ukBankHolidays = newDateSet([]string{
...
func newDateSet(dates []string) map[string]bool {
...
func isBankHoliday(t time.Time) bool {
	return ukBankHolidays[t.Format("2006-01-02")]
}

func isNonWorkingDay(t time.Time) bool {
	return isWeekend(t) || isBankHoliday(t)
}
